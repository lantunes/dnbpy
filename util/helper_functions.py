...
def to_state_action_pair_symmetries_input_channel(board_size, state, action,num_input_channels):
    #state_edge_matrix = convert_board_state_to_edge_matrix(board_size, state)
    num_channels = num_input_channels
    state_edge_matrix = state[0]
    action_edge_matrix = convert_board_state_to_edge_matrix(board_size, action)
    # I symmetry
    state_i = np.array(state_edge_matrix)
    action_i = np.array(action_edge_matrix)
    ...
    if rows == cols:
        ...
        action_s = np.flip(action_i, 1)
        new_state_s=[]
        for n in range(0,num_channels): new_state_s.append(np.flip(state[n],1))
        ...
        new_state_r = []
        for n in range(0, num_channels): new_state_r.append(np.rot90(state[n],1))
        ...
        action_r2 = np.rot90(action_i, 2)
        new_state_r2 = []
        for n in range(0, num_channels): new_state_r2.append(np.rot90(state[n],2))
        ...
        action_r3 = np.rot90(action_i, 3)
        new_state_r3 = []
        for n in range(0, num_channels): new_state_r3.append(np.rot90(state[n], 3))
        ...
        action_sr = np.rot90(action_s, 1)
        new_state_sr = []
        for n in range(0,num_channels): new_state_sr.append(np.rot90(new_state_s[n], 1))
        ...
        action_sr2 = np.rot90(action_s, 2)
        new_state_sr2 = []
        for n in range(0, num_channels): new_state_sr2.append(np.rot90(new_state_s[n], 2))


        # SR^3 symmetry
        action_sr3 = np.rot90(action_s, 3)
        new_state_sr3 = []
        for n in range(0, num_channels): new_state_sr3.append(np.rot90(new_state_s[n], 3))
...
def append_transitions(states, actions, outcomes, all_transitions, use_symmetries, board_size,num_input_channels):
    for i, _ in enumerate(actions):
        state = states[i] #this is itself another list of states
        action = actions[i]
        reward = outcomes[i]
        if use_symmetries:
            state_action_symmetries = to_state_action_pair_symmetries_input_channel(board_size, state, action,num_input_channels)
